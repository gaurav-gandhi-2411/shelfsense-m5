...
class TestSegmentCrossesBar:
    ...
    def test_vertical_arrow_above_bar_no_cross(self):
        ...
        assert not _segment_crosses_bar((0.0, 1.04), (0.0, 0.815), bar_x=0.0, bar_h=0.8, bar_w=0.62)
    ...
    def test_arrow_passes_through_bar_body(self):
        ...
        assert _segment_crosses_bar((1.0, 0.5), (1.0, 0.3), bar_x=1.0, bar_h=0.9, bar_w=0.62)

    def test_arrow_misses_bar_horizontally(self):
        """Arrow at x=3 does not cross bar at x=0."""
        assert not _segment_crosses_bar((3.0, 1.04), (3.0, 0.60), bar_x=0.0, bar_h=0.8, bar_w=0.62)
    ...
    def test_diagonal_arrow_clears_intermediate_bar(self):
        ...
        assert not _segment_crosses_bar((2.0, 1.04), (3.0, 0.91), bar_x=2.0, bar_h=0.86, bar_w=0.62)
    ...
    def test_diagonal_arrow_crosses_tall_intermediate_bar(self):
        ...
        assert _segment_crosses_bar((0.0, 1.04), (3.0, 0.50), bar_x=1.5, bar_h=0.85, bar_w=0.62)
...
class TestCalloutInTopMargin:
    ...
    def test_multiple_callouts_stacked_no_violations(self):
        ...
        canvas = ChartCanvas(figsize=(16, 8), title="T", ylabel="Y", top_margin_pct=0.20)
        x = np.arange(5, dtype=float)
        y = np.array([0.85, 0.70, 0.60, 0.55, 0.65])
        canvas.add_bars(x, y, colors=[BLUE] * 5)
        canvas.set_ylim(0.40, 1.20)
        canvas.add_callout(
            target_x=0, target_y=canvas.bar_top_for_arrow(0), text="First", color=BLUE
        )
        canvas.add_callout(
            target_x=2, target_y=canvas.bar_top_for_arrow(2), text="Second\ntwo lines", color=GREEN
        )
        canvas.add_callout(
            target_x=4, target_y=canvas.bar_top_for_arrow(4), text="Third", color=RED
        )
        violations = canvas.validate()
        assert violations == [], f"Unexpected violations: {violations}"
        import matplotlib.pyplot as plt
...
class TestValidateCatchesOverlap:
    ...
    def test_below_margin_callout_is_detected(self):
        ...
        canvas = ChartCanvas(figsize=(10, 6), title="T", ylabel="Y", top_margin_pct=0.18)
        x = np.array([0.0, 1.0])
        ...
        assert below_margin, f"Expected a 'below margin boundary' violation, got: {violations}"
...
class TestValueLabelZoneCollision:
    ...
    def test_arrow_tip_inside_label_zone_is_detected(self):
        ...
        canvas.add_callout(
            target_x=0.0,
            target_y=0.805,  # inside value-label zone of bar at x=0
            text="Tip in label zone",
        )
        violations = canvas.validate()
        label_zone_viol = [v for v in violations if "value-label zone" in v]
        assert label_zone_viol, f"Expected Rule 2b violation (tip in label zone), got: {violations}"
        import matplotlib.pyplot as plt
    ...
    def test_arrow_above_label_zone_is_clean(self):
        ...
        assert label_viol == [], (
            f"bar_top_for_arrow() should produce no label violations: {violations}"
        )
        import matplotlib.pyplot as plt
...
class TestTitleClearance:
    ...
    def test_callout_within_margin_clears_title(self):
        ...
        canvas.add_callout(
            target_x=1.0,
            target_y=canvas.bar_top_for_arrow(1.0),
            text="Normal callout\nin margin zone",
        )
...
class TestSameRowPlacement:
    ...
    def test_same_row_does_not_advance_cursor(self):
        ...
        canvas.add_callout(
            target_x=2.0,
            target_y=canvas.bar_top_for_arrow(2.0),
            text="First same-row callout",
            same_row=True,
        )
        canvas.add_callout(
            target_x=8.0,
            target_y=canvas.bar_top_for_arrow(8.0),
            text="Second same-row callout",
            same_row=True,
        )
...
class TestLegendOutsideBody:
    ...
    def test_legend_is_added_and_placed_outside(self):
        ...
        canvas = ChartCanvas(figsize=(10, 6), title="T", ylabel="Y")
        canvas.add_bars(np.array([0.0, 1.0]), np.array([0.5, 0.7]), colors=[BLUE, RED])
        canvas.set_ylim(0.30, 1.00)
        handles = [
