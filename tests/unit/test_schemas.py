...
def _make_raw_sales(n: int = 3) -> pd.DataFrame:
    return pd.DataFrame({
        "id": [f"FOODS_1_001_CA_1_evaluation"] * n,
...
def _make_feature(n: int = 5) -> pd.DataFrame:
    lag_cols = {f"lag_{k}": np.full(n, np.nan, dtype="float32")
                for k in (7, 14, 28, 56, 91, 182, 364)}
    roll_cols = {f"roll_{s}_{w}": np.full(n, 1.0, dtype="float32")
