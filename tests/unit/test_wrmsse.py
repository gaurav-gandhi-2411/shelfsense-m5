"""
Unit tests for the WRMSSE evaluator (shelfsense.evaluation.wrmsse).
"""
import numpy as np
import pandas as pd
import pytest

from shelfsense.evaluation.wrmsse import (
    LEVEL_SPECS,
    HORIZON,
    build_scales,
...
def test_weights_sum_to_one():
    ...
    for level, (keys, w) in w_dict.items():
        assert np.isclose(w.sum(), 1.0, atol=1e-9), f'Level {level}: weights sum to {w.sum():.6f}'
...
def test_perfect_forecast_gives_zero_wrmsse():
    ...
    preds   = actuals.copy()
    wrmsse, level_scores = compute_wrmsse(preds, actuals, df.copy(), prices, cal, last_train_day=n_train - HORIZON)
...
def test_wrmsse_positive_for_nonzero_error():
    ...
    actuals = df[[f'd_{d}' for d in range(n_train - HORIZON + 1, n_train + 1)]].values.astype(float)
    preds   = actuals + 1.0
    wrmsse, _ = compute_wrmsse(preds, actuals, df, prices, cal, last_train_day=n_train - HORIZON)
    assert wrmsse > 0.0
...
def test_twelve_level_breakdown():
    ...
    wrmsse, level_scores = compute_wrmsse(preds, actuals, df, prices, cal, last_train_day=n_train - HORIZON)
...
def test_submission_to_matrix_alignment():
    ...
    sub_rows = pd.DataFrame(np.column_stack([df['id'].values, vals]), columns=['id'] + fcols)
